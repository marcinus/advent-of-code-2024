...
static def findSolution(A, B, P) {
    def pX = B[1] * P[0] - B[0] * P[1]
    def pY = A[1] * P[0] - A[0] * P[1]
    def xyC = A[1] * B[0] - A[0] * B[1]
    if (-pX % xyC == 0 && pY % xyC == 0) return [-pX / xyC, pY / xyC]
    [0, 0]
}
...
static def parse(List<String> input) {
    ...
    input.collate(4).collect {
        ...
        [
                [buttonA[1] as long, buttonA[2] as long],
                [buttonB[1] as long, buttonB[2] as long],
                [prize[1] as long, prize[2] as long]
        ]
    }
}
...
assert 480L == solve(sampleInput.split('\n') as List)
